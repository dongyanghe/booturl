...
## Object
#### 属性
Object.enumerrable: Boolean 是否可枚举
#### 操作函数
*   Object.values(obj | array) 以for in 循环的顺序返回obj | array值的一维数组[value]
*   Object.entries(obj) 以for in 循环的顺序返回obj | array键值对的二维数组：[[key, value]]
### 字符串
...
*   replace() – 用来查找匹配一个正则表达式的字符串，然后使用新字符串代替匹配的字符串，不改变原字符串。
...
* 增
    padStart(targetLength, padString = ' ') - 填充字符串的开始，以便生成的字符串达到给定的长度，默认空格
    padEnd(targetLength, padString = ' ') - 填充字符串的结束，以便生成的字符串达到给定的长度，默认空格
* 删
* 改
* 查
* 存
...
### SharedArrayBuffer
一个通用的，固定长度的原始二进制数据缓冲区,实现共享内存，通过 atomic 操作来进行同步。
...
function pivotIndex(nums) {
        ...
        for (let i in nums) {
          ...
          if (index != 0 && index != length - 1) {
            ...
            const leftListNum = eval(leftList.join('+'));
            const rightList = nums.slice(index + 1, length);
            const rightListNum = eval(rightList.join('+'));
            console.log(nums, index, leftList, rightList);
            if (leftListNum == rightListNum) {
